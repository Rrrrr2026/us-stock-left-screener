...
本次升级修复了一批数据缺陷并扩展了算法，变更如下：

### 8.1 数据修复
| 问题 | 根因 | 修复 |
|---|---|---|
| 股息率显示 68% / 94% 等离谱值 | yfinance ≥0.2.54 起 `dividendYield` 改为百分数口径，老代码的 `<1 则 ×100` 启发式把真实 <1% 的股息率放大100倍 | 首选 `dividendRate/现价` 直接算（单位无歧义），兜底用 `trailingAnnualDividendYield` 判别口径；>30% 置空 |
| 板块错标（Altria=Health Care 等）与缺失（—） | NASDAQ 名单用自家分类，质量差且 ~10% 行留空 | 候选股一律用 Yahoo `info.sector`（GICS口径）覆盖，NASDAQ 仅兜底 |
| 全横杠基本面行反而得中性 50 分 | 限频时空壳 info 被缓存一整天 + 缺数据不折价 | 空壳 info 不再缓存；基本面核心字段全缺 → 上限45再扣5 |
| 数据日期误导（显示运行日而非行情日） | `run_date=今天`，但 08:00 跑的是前一交易日收盘 | 新增 `meta.data_date` = SPY 最新bar日期，前端优先展示 |
| KDJ 列几乎全是"金叉" | 把 K>D 持续状态当"金叉"事件 | 只有真实交叉（前一根K≤D，本根K>D）才标金叉，持续状态标 偏多/偏空 |
| 放量确认失真 | 量比分母含当日（2倍量只显示1.9） | 分母改为前20日均量 |
| 前低支撑"已破位仍加分" | 距离用绝对值判断，价格跌破前低4%仍算贴近 | 与均线/通道同口径：最多允许下方1% |
| 破位参考位贴着两年前的深低点 | 用全历史枢轴低点取 min | 只看现价下方10%以内的支撑 |
| 优先股/挂牌债券/SPAC单位混入股票池 | NASDAQ 给这类行填发行人市值 | 名称正则剔除（保留 Preferred Bank 这类真公司） |
| RSI 全涨窗口返回 NaN | 分母为0被替换成 NaN | 补数学极限：全涨=100，全平=50 |

### 8.2 扫描面扩容
- 市值下限 $1B → **$100M**：扫描 ~2800 → **~4100+ 只**；质量闸门交给"近20日日均成交额≥$5M"流动性过滤。
- 基本面阶段 Top 300 → **400**，主榜 200 → **250**。

### 8.3 技术打分新增 3 个信号（权重和 5.1 → 6.4）
1. **支撑强度** `supp_strength=0.4`：主支撑区（±1.5%）近一年被触碰次数，触碰≥2次开始加分，越厚越可信。
2. **趋势规整** `trend_regime=0.5`：MA250 上方且 MA250 上行的回踩（stage-2 回调）满分；半符合减半。区分"强势股洗盘"与"破位下行"。
3. **相对强度** `rel_strength=0.4`：近60日跑赢 SPY 的幅度（超额>0 才加分，10%封顶）。
另：RSI 底背离并入超跌信号组（+0.25×1.2）；`强左侧` 技术门槛 2.0 → 2.2 同步上调。

### 8.4 基本面打分升级
- 新增 **FCF收益率**（自由现金流/市值）：≥6% 加6，≥3% 加3，为负扣4。
- **数据覆盖折价**：核心5字段（ROE/PE/净利同比/毛利/杠杆）全缺 → 分数上限45再扣5；仅1个 → 扣3。

### 8.5 🎯 买卖点建议（新模块 `tradeplan.py`）
对每只候选股，用它**自己的历史**做"支撑回踩"事件回测：

```
事件定义（历史上的某一天）:
  ① 距近120日高点回撤 ≥8%（处于回调中）
  ② RSI14 ≤ 55（偏弱）
  ③ 当日最低价触到"当时已知"的支撑位 ±2%（MA60/120/250 或已确认的枢轴低点）
  ④ 收盘未破支撑 3% 以上；两次事件至少间隔8个交易日

事件结局（往后看60个交易日）:
  先碰止损（支撑下方 1.8×ATR，夹在5%~15%）→ 记止损
  先到目标涨幅 g ∈ {3,5,8,10,15,20,30}% → 记命中（同日双触发按止损保守处理）
  窗口截断未出结果 → 按未达标保守计

胜率估计（单股样本少，贝叶斯收缩）:
  p(g) = (本股命中数 + 12 × 全池先验命中率) / (本股事件数 + 12)
  全池先验 = 当日全部候选的事件汇总（数千次事件）
```

输出（详情弹窗新增"🎯 买卖点建议"卡片）：
- **建议买入区**：支撑位 ±0.4~0.5×ATR（已失守支撑的深跌股改用现价锚定、注明"现价分批"）
- **止损位**：ATR止损与破位位取更深者（≤15%），跌破即离场
- **目标梯子**：稳健（胜率≥75%）/ 基准（≥50%）/ 进取（≥30%）三档 + 完整7档表，每档给出目标价、历史胜率、中位到达天数 —— 形如"+10% 胜率 40%"
- **盈亏比** = 基准目标涨幅 / 止损距离；并展示样本量（本股N次 + 全池M次）

> ⚠️ 诚实声明：胜率是**历史回测频率**（60日窗口、先到先算），不是对未来的保证；样本<5次的股票主要靠全池先验，参考价值降低（前端会黄字提示）。


## 9. v2.1 增补（2026-08-08 第二轮）

- **🚀 蓄势待发桶**（与 dip 桶同构、独立于 tech_score）：深回调(箱体上沿距250日高点≥25%、高点≥40根bar前) + 横盘收敛(近30根bar振幅≤16%) + 贴近箱体上沿(≤5%且在上半部)。确认信号：波动挤压(布林带宽年内40%分位以下)/MACD走强/KDJ多头/站上MA60/放量上攻。**买卖点为突破型剧本**：买点=放量突破箱体上沿、止损=跌回箱体下沿、目标=箱体高度量度目标（明确不标胜率——支撑回踩的历史胜率描述不了突破交易）。
- **主表新列**（所属板块右侧）：市场地位（NASDAQ 153个细分行业内市值排名/份额，垄断力代理，👑=第一且份额≥15%）、近四季净利同比、近四季归母同比（分层口径 TTM/单季/年度，带财报期连续性校验，归母与净利各标各的口径）、增长持续性（近4财年趋势×近4季环比交叉判断：持续增长/拐点向上/单季脉冲/增速回落/增长下滑）、市盈率TTM移至此处。
- **历史快照回看**：每日导出瘦身版 `docs/history/day_<date>.json`（无K线/档案，~1MB），前端日期下拉切换任意历史日榜单（横幅提示、防乱序、失败回退）；保留90天。
- 弹窗K线/指标图移至总览页底部；深度档案全空自动重试修复"公司与业务/现金流全暂无"问题。
