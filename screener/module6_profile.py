...
def _cash_insights(cf: dict) -> list:
    ...
    if ocf is not None and ni is not None and ni > 0:
        ...
        if r < 0.7:
            notes.append({"level": "warn",
                          "text": f"{yr}年经营现金流仅为净利润的{r*100:.0f}% —— 利润未充分转化为现金(应收/存货占款?), 盈利质量需警惕"})
        elif r > 1.1:
            notes.append({"level": "good",
                          "text": f"{yr}年经营现金流为净利润的{r*100:.0f}% —— 利润含金量高"})
    if ni is not None and ni < 0 and ocf is not None and ocf > 0:
        notes.append({"level": "info",
                      "text": f"{yr}年账面亏损但经营现金流为正(${ocf:,.0f}M) —— 亏损或主要来自非现金项目(摊销/减值)"})

    # 2) 自由现金流
    if fcf is not None and fcf < 0:
        notes.append({"level": "warn",
                      "text": f"{yr}年自由现金流为负(${fcf:,.0f}M) —— 经营造血不足以覆盖资本开支, 需外部融资"})

    # 3) 大额收购
    if acq is not None and acq < 0 and ocf and ocf > 0 and abs(acq) >= 0.3 * ocf:
        notes.append({"level": "warn",
                      "text": f"{yr}年收购支出${abs(acq):,.0f}M, 相当于经营现金流的{abs(acq)/ocf*100:.0f}% —— 关注商誉与整合风险"})
    ...
    if ret > 0 and fcf is not None:
        if fcf > 0 and ret > fcf and (debt_net or 0) > 0:
            notes.append({"level": "warn",
                          "text": f"{yr}年回购+分红(${ret:,.0f}M)超过自由现金流(${fcf:,.0f}M)且当年净举债 —— 借钱回馈股东, 不可持续"})
        elif fcf > 0 and ret > 0.9 * fcf:
            notes.append({"level": "info",
                          "text": f"{yr}年回购+分红(${ret:,.0f}M)几乎用尽自由现金流 —— 留给扩张/还债的余地小"})
        elif fcf > 0:
            notes.append({"level": "good",
                          "text": f"{yr}年以${ret:,.0f}M回馈股东(回购${abs(buyback or 0):,.0f}M+分红${abs(dividend or 0):,.0f}M), 自由现金流覆盖充分"})
    ...
    if len(capex_arr) >= 3 and capex is not None and ocf and ocf > 0:
        if abs(capex) > 0.8 * ocf:
            notes.append({"level": "info",
                          "text": f"{yr}年资本开支${abs(capex):,.0f}M, 占经营现金流的{abs(capex)/ocf*100:.0f}% —— 重资产扩张期, 关注回报率"})
    return notes
